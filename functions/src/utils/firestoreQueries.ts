 ...
 */
export async function getAllActiveProjectsWithPositions(): Promise<(ProjectWithId & { positions: PositionWithId[] })[]> {
...
export async function getProjectsMatchingStudentInterests(studentId: string): Promise<ProjectWithId[]> {
  ...
  const matchingProjects = matchingProjectsSnapshot.docs
    ...
    .filter(project => {
      ...
      const projectKeywords = (project as any).keywords || [];
      return projectKeywords.some((keyword: string) => interests.includes(keyword));
    });
