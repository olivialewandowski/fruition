...
export const onProjectCreate = functions.firestore
  ...
  .onCreate(async (snapshot: functions.firestore.QueryDocumentSnapshot, context: functions.EventContext) => {
    ...
    try {
      ...
      // Cast to any to access keywords property which might not be in the type definition
      const projectKeywords = (projectData as any).keywords || [];
...
export const onApplicationUpdate = functions.firestore
  .document('projects/{projectId}/positions/{positionId}/applications/{applicationId}')
